...
- (void)initWebview:(FlutterMethodCall*)call withResult:(FlutterResult)result {
    ...
    __block NSString *cookieSource;
    NSMutableArray *temArray = [NSMutableArray array];
    [cookies enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
           NSDictionary *dic = obj;
           cookieSource = [NSString stringWithFormat:@"document.cookie= '%@=%@'",dic[@"k"], dic[@"v"]];
        [temArray addObject:cookieSource];
       }];
    
    WKUserScript *cookieScript = [[WKUserScript alloc] initWithSource:[temArray componentsJoinedByString:@";"] injectionTime:WKUserScriptInjectionTimeAtDocumentStart forMainFrameOnly:NO];
    [userContentController addUserScript:cookieScript];
    WKUserScript *script = [[WKUserScript alloc] initWithSource:@"window._dswk=true;" injectionTime:WKUserScriptInjectionTimeAtDocumentStart forMainFrameOnly:YES];
...
- (void)webView:(WKWebView *)webView runJavaScriptTextInputPanelWithPrompt:(NSString *)prompt defaultText:(nullable NSString *)defaultText initiatedByFrame:(WKFrameInfo *)frame completionHandler:(void (^)(NSString * _Nullable result))completionHandler {
    ...
    if ([prompt hasPrefix:prefix]) {
        ...
        NSString *result = nil;
        [channel 
         invokeMethod:@"onJavascriptCalled" 
...
- (void)userContentController:(nonnull WKUserContentController *)userContentController didReceiveScriptMessage:(nonnull WKScriptMessage *)message { 
    ...
    NSLog(@"js调用原生的方法:%@; 消息体:%@",message.name,message.body);
}
